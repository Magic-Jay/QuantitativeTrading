                ...
                if (!antithetic)
                {
                    ...
                    for (int i = 0; i < trials; i++)
                    {
                        ...
                        if (controlVariate)
                        {
                            ...
                            pricesByTrial[0, i] = Math.Max(lookBackMaxs[i] - k, 0) - controlVariatePathCall[i, steps - 1];
                            pricesByTrial[1, i] = Math.Max(k - lookBackMins[i], 0) - controlVariatePathPut[i, steps - 1];
                        }
                        else
                        {
                            pricesByTrial[0, i] = Math.Max(lookBackMaxs[i] - k, 0); //Max - K for Call
                            pricesByTrial[1, i] = Math.Max(k - lookBackMins[i], 0); //K - Min for Put
                        }
                ...
                else
                {
                    for (int i = 0; i < trials; i++)
                    {
                        ...
                        if (controlVariate)
                        {
                            ...
                            pricesByTrial[0, i] = 0.5 * (Math.Max(lookBackMaxs[i] - k, 0) - controlVariatePathCall[i, steps - 1]
                                + Math.Max(lookBackMaxsAntithetic[i] - k, 0) - controlVariatePathAntiTheticCall[i, steps - 1]);

                            pricesByTrial[1, i] = 0.5 * (Math.Max(k - lookBackMins[i], 0) - controlVariatePathCall[i, steps - 1]
                                + Math.Max(k - lookBackMinsAntithetic[i], 0) - controlVariatePathAntiTheticCall[i, steps - 1]);
                        }
                        else
                        {
                            //antithetic price
                            pricesByTrial[0, i] = 0.5 * (Math.Max(lookBackMaxs[i] - k, 0) + Math.Max(lookBackMaxsAntithetic[i] - k, 0));

                            pricesByTrial[1, i] = 0.5 * (Math.Max(k - lookBackMins[i], 0) + Math.Max(k - lookBackMinsAntithetic[i], 0));
