...
namespace Eagle.Business_Logic
{
    public class DigitalOption
    {
        ...
        public static Dictionary<string, double> GetPrices(int steps, int trials, double s, double k, double t, double sig, double r, double rebate)
        {
            ...
            double[,] digitalPayoff = new double[2, trials];
            double[,] digitalPayoffAntithetic = new double[2, trials];
            ...
            try
            {
                if (!antithetic)
                {
                    ...
                    for (int i = 0; i < trials; i++)
                    {
                        if (simulationRegular[i, steps - 1] - k > 0)
                        {
                            digitalPayoff[0, i] = rebate;
                        }                            
                        else
                        {
                            digitalPayoff[0, i] = 0;
                        ...
                        if (k - simulationRegular[i, steps - 1] > 0)
                        {
                            digitalPayoff[1, i] = rebate;
                        }                           
                        else
                        {
                            digitalPayoff[1, i] = 0;
                        ...
                        if (controlVariate)
                        {
                            ...
                            pricesByTrial[0, i] = Math.Max(digitalPayoff[0, i] - k, 0) - controlVariatePathCall[i, steps - 1];
                            pricesByTrial[1, i] = Math.Max(k - digitalPayoff[1, i], 0) - controlVariatePathPut[i, steps - 1];
                        }
                        else
                        {
                            pricesByTrial[0, i] = digitalPayoff[0, i];
                            pricesByTrial[1, i] = digitalPayoff[1, i];
                        }
                ...
                {                    
                    for (int i = 0; i < trials; i++)
                    {
                        ...
                        //antithetic
                        if (simulationAntithetic[i, steps - 1] - k > 0)
                            digitalPayoffAntithetic[0, i] = rebate;
                        else
                            digitalPayoffAntithetic[0, i] = 0;

                        if (k - simulationAntithetic[i, steps - 1] > 0)
                            digitalPayoffAntithetic[1, i] = rebate;
                        else
                            digitalPayoffAntithetic[1, i] = 0;

                        if (controlVariate)
                        {
                            //antithetic and control variate price
                            pricesByTrial[0, i] = 0.5 * (digitalPayoff[0, i] - controlVariatePathCall[i, steps - 1]
                                + digitalPayoffAntithetic[0, i] - controlVariatePathAntiTheticCall[i, steps - 1]);

                            pricesByTrial[1, i] = 0.5 * (digitalPayoff[1, i] - controlVariatePathCall[i, steps - 1]
                                + digitalPayoffAntithetic[1, i] - controlVariatePathAntiTheticCall[i, steps - 1]);
                        }
                        else
                        {
                            //antithetic price
                            pricesByTrial[0, i] = 0.5 * (digitalPayoff[0, i] + digitalPayoffAntithetic[0, i] );

                            pricesByTrial[1, i] = 0.5 * (digitalPayoff[1, i] + digitalPayoffAntithetic[1, i]);
